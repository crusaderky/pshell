...
def test_ignore_readonly1(tmpdir):
    ...
    sh.remove("%s/foo" % tmpdir, force=False, recursive=True, ignore_readonly=True)
...
def test_backup(tmpdir):
    ...
    # Manual extension
    new_fname = sh.backup("$UNITTEST_BASH/test", suffix="bak", action="copy")
...
def test_symlink(tmpdir):
    ...
    # abspath = False
    sh.symlink("$UNITTEST_BASH/test_ln1", "$UNITTEST_BASH/test_ln3", abspath=False)
    assert (
    ...
    # abspath = True
    sh.symlink("$UNITTEST_BASH/test_ln1", "$UNITTEST_BASH/test_ln3", abspath=True)
    assert (
        subprocess.check_output(
            "ls -l %s/test_ln3 | awk '{print $NF}'" % tmpdir, shell=True
        ).decode("utf-8")
    ...
    # no force
    with pytest.raises(FileExistsError):
        sh.symlink("$UNITTEST_BASH/test_ln2", "$UNITTEST_BASH/test_ln3", force=False)

    # force must work only to override another symlink,
    # NOT another regular file
    with pytest.raises(FileExistsError):
        sh.symlink("$UNITTEST_BASH/test_ln1", "$UNITTEST_BASH/test_ln2", force=True)

    # force; old symlink is different
    sh.symlink("$UNITTEST_BASH/test_ln2", "$UNITTEST_BASH/test_ln3", force=True)
    assert (
        subprocess.check_output(
            "ls -l %s/test_ln3 | awk '{print $NF}'" % tmpdir, shell=True
        )
...
def test_mkdir(tmpdir):
    ...
    with pytest.raises(FileExistsError):
        sh.mkdir("$UNITTEST_BASH/test_mkdir_file", force=True, parents=False)

    # Missing middle path
    with pytest.raises(FileNotFoundError):
        sh.mkdir("$UNITTEST_BASH/middle/test_mkdir", parents=False, force=False)

    sh.mkdir("$UNITTEST_BASH/middle/test_mkdir", parents=True, force=False)
